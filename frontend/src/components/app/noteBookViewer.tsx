  ...
  const renderOutput = (output: any, outputIndex: number) => {
    ...
    if (output.output_type === 'execute_result' || output.output_type === 'display_data') {
      ...
      if (data?.['text/html']) {
        return (
          <div className="bg-white dark:bg-gray-900 border rounded p-3">
            <div className="text-xs text-gray-500 mb-2 uppercase tracking-wide">HTML Output</div>
            <div dangerouslySetInnerHTML={{ __html: getSourceText(data['text/html']) }} />
          </div>
        );
      }
      if (data?.['text/plain']) {
      ...
      if (data?.['image/png']) {
        return (
          <div className="bg-white dark:bg-gray-900 border rounded p-3">
            <div className="text-xs text-gray-500 mb-2 uppercase tracking-wide">Image Output</div>
            <img src={`data:image/png;base64,${data['image/png']}`} alt="Output" className="max-w-full h-auto" />
          </div>
        );
      }
    }
    ...
    return (
      <div className="bg-gray-100 dark:bg-gray-800 p-3 text-sm">
        <div className="text-xs text-gray-500 mb-1">Unknown output type: {output.output_type}</div>
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-600">Markdown Cell</span>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => toggleCellCollapse(index)}>
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-600">Code Cell</span>
                      <Badge variant="outline" className="text-xs">{index + 1}</Badge>
                      {cell.execution_count && (
                        <Badge variant="secondary" className="text-xs">[{cell.execution_count}]</Badge>
                      )}
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm">
